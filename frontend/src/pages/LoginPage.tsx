...
export default function LoginPage() {
  ...
  // Modal mode: public site, navigated with backgroundLocation in state
  const state = location.state as { backgroundLocation?: Location } | null
  const isModal = !APP_MODE && !!state?.backgroundLocation
  ...
  // ── Form card (shared between modal and full-page) ──────────────────────────
  const formCard = (
  ...
  // ── MODAL mode (public site, navigated from navbar) ─────────────────────────
  if (isModal) {
    return (
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={dismiss}>
        ...
        <button
          onClick={dismiss}
          className="absolute top-5 right-5 z-10 w-9 h-9 rounded-full bg-[var(--bg-card)] border border-[var(--border)] flex items-center justify-center text-[var(--text-secondary)] hover:text-white hover:border-[var(--accent)] transition-all"
          aria-label="Close"
        >
          <X size={18} />
        </button>

        {/* Card — stop backdrop click from propagating */}
  ...
  // ── FULL-PAGE mode (portal mode, or direct /login URL without state) ─────────
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] flex flex-col">
      ...
      <div className="relative z-10 flex flex-col min-h-screen items-center justify-center p-4">
        {!APP_MODE && (
          <button
            onClick={() => navigate(-1)}
            className="absolute top-6 left-6 flex items-center gap-2 text-[var(--text-secondary)] hover:text-white transition-colors text-sm"
          >
            ← Back to site
          </button>
        )}
